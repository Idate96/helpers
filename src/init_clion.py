...
def create_script(project_path, project_name, catkin_ws_path, package_names, force):
    ...
    if catkin_ws_path is None:
        given_explicit_catkin_ws_path = False
        ...
        given_explicit_catkin_ws_path = True
    ...
    # Check to see if we should load CMAKE_PREFIX_PATH from the environment or from a hardcoded list in the CMakeLists.txt
        # If either the CMAKE_PREFIX_PATH is only /opt/ros/DISTRO (so a workspace hasn't been sourced) or -w set an explicit catkin_ws, then use the env variables
        # If a workspace was sourced and the -w option wasn't used, use the current environment CMAKE_PREFIX_PATH (which is assumed to be correct)
    cmake_prefix_paths = os.environ['CMAKE_PREFIX_PATH'].split(':')
    if (len(cmake_prefix_paths) == 1 and cmake_prefix_paths[0][0:9] == '/opt/ros/') or given_explicit_catkin_ws_path :
        print('The CMAKE_PREFIX_PATH is set by the environment variable')
        cmakelists.write("\n## !!! The CMAKE_PREFIX_PATH will be taken from Environment Variables, which you can set in Clion per project !!!\n")
    else:
        print('The CMAKE_PREFIX_PATH is set in the CMakeLists to ' + ', '.join(cmake_prefix_paths))
        cmakelists.write("\n## Load the CMAKE_PREFIX_PATH, which presumably matches the original catkin config\n")
        for path in cmake_prefix_paths:
            cmakelists.write("list(APPEND CMAKE_PREFIX_PATH \"" + path + "\")\n")
    ...
    for package_name in package_names:
        ...
        packageDeps = str(subprocess.check_output(bashCommand.split()))
        packageFound = '\\n' + package_name + '\\n'
        if len(packageDeps.split(packageFound, 1)) > 1:
            packageDeps = packageDeps.split(packageFound, 1)[1]
            if len(packageDeps.split('build_depend:\\n', 1)) > 1:
    ...
    for dependency in dependencies:
        ...
        path = str(subprocess.check_output(locateCommand.split()))
        path = path.split('\\n')[0]
        path = path.split('b\'')[1]
        os.chdir(path)
        if os.system('git rev-parse 2> /dev/null > /dev/null') == 0:
            ...
            repo_folder = path
            print('adding package ' + dependency + ' without git repo: ' + repo_folder)
